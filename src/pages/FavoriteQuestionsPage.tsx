...
import {
  ...
  toggleFavoriteUtil, addWrong, removeWrong, FAVORITES_KEY,
...
export default function FavoriteQuestionsPage() {
  ...
  /* Load data - filter favorite questions */
  useEffect(() => {
    fetch('/questions/questions_1_50.json')
      ...
      .then((data) => {
        ...
        const favIds: number[] = JSON.parse(localStorage.getItem(FAVORITES_KEY) || '[]');
        const favSorular = all.filter((s: Soru) => favIds.includes(s.soru_no));
        setSorular(favSorular);
  ...
  const handleToggleFavorite = useCallback(() => {
    ...
    // Refresh sorular list if unfavorited
    const newSorular = sorular.filter((s) => newFavs.includes(s.soru_no));
    if (newSorular.length !== sorular.length) {
      setSorular(newSorular);
      if (quizState.currentQuestionIndex >= newSorular.length) {
        setQuizState((s) => ({ ...s, currentQuestionIndex: Math.max(0, newSorular.length - 1) }));
      }
    }
  }, [currentSoru, sorular, quizState.currentQuestionIndex]);
  ...
  if (total === 0) {
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center bg-navy-900 px-6">
        <Bookmark className="w-16 h-16 text-gold-500 mb-4" />
        <h2 className="text-xl font-bold text-white mb-2">Favori Sorular</h2>
        <p className="text-gray-400 text-center">Henüz favorilere eklediğiniz soru yok.</p>
        <button
          onClick={() => navigate(`/packages/${params.id || 'stab_2024_02'}`)}
          className="mt-6 h-12 px-6 bg-gold-500 hover:bg-gold-400 text-navy-950 font-semibold rounded-xl transition-colors"
        >
          Pakete Dön
        </button>
      </div>
